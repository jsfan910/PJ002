...
| US-010 | staging 部署 | P0 | UC-010、UC-011 | BE-02、BE-07、BE-08、OPS-01、OPS-02、OPS-03、OPS-04 | `GET /health`（**唯一未保護路徑**，O-005） | TC-078 ~ TC-084、TC-089、TC-090、TC-095、TC-105 | **staging 已於 2026-09-19 部署（T-0027），原 8 條阻擋逐條給結果**：**通過 8**（其中 6 條於 T-0030 判定，TC-089 於 T-0037 補判，TC-080 由使用者於 2026-09-20 Gate 2 r2 裁決②追認，詳見本格末段）—— TC-078（UAT r4 於 staging 完成 US-001~007）、TC-079（實測 **302**，Leader 2026-09-19T17:30:10 裁決接受 302 等效；規格文字落差記 **D-016**）、TC-081（T-0027 實際完成一次 staging 部署佐證）、TC-084（回滾演練 9.689s／10.355s，`06` §5.4 已回填）、TC-090（部署期間 228 樣本全 200、最長連續失敗 **0 秒**）、TC-105（`/health` 200／`/` 401／帶憑證 200，HTTPS）；**TC-080：通過（追認）** —— 使用者於 Gate 2 報告 r2 裁決②（2026-09-20）追認三項互相獨立的間接證據（部署期間 `/health` 零中斷、Neon 與 Cloud Run 運算分離、回滾只切 revision 不動資料層）及本機等效 TC-093 通過；「同批資料跨部署前後」的直接比對留 P1 staging 測試輪補做（非擋關）。TC-082／083／095 續維持通過（T-0030）。<br>**TC-089：通過（T-0037，2026-09-20 判讀）** —— NFR-003 的 24 小時窗已走完，以 Cloud Monitoring uptime check `todo-app-health-aMAlP5dfKv0` 於窗 `2026-09-19T09:55:07Z ~ 2026-09-20T10:04:05Z`（24h08m58s）判讀：**4 個檢查地區各 289 次檢查執行、合計 1156 次取樣全部成功（原始資料點 6936／6936 `true`、`false` 0），成功率 100.0000% ≥ 99% 門檻**；逐地區最大取樣間隔 270~280 秒 < 300 秒週期，無漏採輪次。證據見 `docs/reports/20260920-1806-測試總結-E001-r3.md` §7.1。**本列狀態由「未判定 1」改為「通過 7、部分通過 1」（T-0037），再由使用者裁決②改為「通過 8、部分通過 0」（2026-09-20 Gate 2 r2）** |
