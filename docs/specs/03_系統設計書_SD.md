...
| SD-02 | **P1 資料清空的執行者與時機**：BR-032／Q-011 裁決「導入 P1 時直接清空 staging 既有待辦資料」，但未指定由誰在哪一步執行 | 建議做成 `002_*.sql` migration 的第一行 `TRUNCATE todos;`，與加 `owner_id NOT NULL` 同一個 migration，使其不可能被遺漏。**屬 P1，Gate 1 不受影響** | dev-ops（P1） |
