...
```mermaid
C4Container
    title C4 Level 2 — 容器：E-001（Cloud Run 單一服務 ＋ Neon Postgres）

    Person(user, "一般使用者 / 驗收者", "瀏覽器")
    Person(apiDev, "用戶端開發者", "curl / 未來行動 App")
    System_Ext(monitor, "監測排程器", "GitHub Actions schedule")

    Container_Boundary(cloudrun, "Cloud Run service（單一容器，單一來源，min instances = 0）") {
...
| 雲端／部署 | **Google Cloud Run**（Docker 映像，`min instances = 0`，區域 `asia-east1`），單一服務同時供應前端與 API；映像存 **Artifact Registry** | **ADR-0005**（取代 ADR-0003） |
...
| CI/CD | GitHub Actions（Epic 指定）；以 Workload Identity Federation 認證 GCP | **ADR-0005**（取代 ADR-0003） |
...
| **OPS-03** `deploy-staging` | `infra/cloudrun-service.yaml`（宣告服務名／region／容器埠／資源規格／`min instances = 0`／startup probe 路徑／環境變數**名稱**）＋ `.github/workflows/deploy-staging.yml`：套用 migration → `docker build` → push 至 Artifact Registry（以 commit SHA 標記）→ `gcloud run deploy` → 輪詢 `/health` 直到 200（逾時即失敗）。GCP 認證採 Workload Identity Federation（`06_部署架構與CICD.md` 第 3.2.1 節） | UC-010 | 輸入：main 綠燈。輸出：staging 新 revision | OPS-02, BE-08 |
...
- pino JSON 結構化日誌輸出至 stdout（容器慣例，由 Cloud Logging 收集）。
...
| NFR-ID | 設計對策 | 驗證方式 |
|---|---|---|
| **NFR-001** 效能<br>P95 < 500ms／10 併發 | ① Fastify 低額外負擔 ＋ JSON Schema 序列化；② `pg.Pool`（`max: 5`）避免每請求建連線；③ 索引 `(created_at DESC, id DESC)` 讓清單查詢走索引掃描；④ 應用啟動即建池，抵銷 Neon 冷啟；⑤ **OPS-04 的 5 分鐘監測同時具備保溫效果**，降低 Cloud Run 在 `min instances = 0` 下被回收後的冷啟命中率（冷啟本身僅 **1–3 秒**，ADR-0005） | `autocannon -c 10 -d 60 -H "Authorization: Basic <base64>" $STAGING_BASE_URL/api/v1/todos`，**先暖身 10 秒再開始取樣**（排除 Neon 冷啟與 Cloud Run 容器啟動的離群值）；輸出 P95 存入測試報告。建立/更新/刪除另跑一輪，門檻 P95 < 800ms |
| **NFR-002** 安全<br>五項 | ①HTTPS：Cloud Run 為 `*.run.app` 自動配發憑證，並由 Google 前端將明文 HTTP 請求以 **3xx（實測 302 Found）** 導向 HTTPS（ADR-0005 決定第 7 點及其 T-0040 附註），團隊零程式碼。**判準為「回 3xx 且 `Location` 為對應的 `https://` 網址」，不寫死單一狀態碼**——導向狀態碼是平台行為，團隊零程式碼可改、平台亦未承諾其不變（D-016；Leader 2026-09-19T17:30:10 裁決①接受 302 等效）；②不洩漏內部細節：BE-03 單點剝除堆疊/SQL/路徑，5xx 訊息為固定字面值；③bcrypt cost 12（P1）；④JWT 保護全部待辦端點（P1，BE-13）；⑤前端全面 `textContent`，禁 `innerHTML` | ①`curl -sSI "http://${STAGING_BASE_URL#https://}/health"` **觀察首行為 3xx**（301 或 302 皆合規）**且 `location` 標頭為對應的 `https://` 網址** ＋ 瀏覽器檢視憑證；②對 5 種錯誤情境打 API 檢視回應本文；③查 `users` 表 `password_hash` 欄位值；④無 token 呼叫五個端點皆 401；⑤建立標題為 `<script>alert(1)</script>` 的待辦，頁面無彈窗且文字原樣顯示。**另：`grep -rn "innerHTML" public/` 應無輸出**；**`curl -sSI -u ... /api/v1/todos \| grep -i access-control-allow-origin` 應無輸出**（AC-009-5 判準，ADR-0004 第 6 點） |
| **NFR-003** 可用性<br>成功率 ≥ 99%；部署不可用 < 60 秒 | ① `/health` 豁免 Basic Auth（BR-017、O-005），監測器不需憑證；② `/health` **不查資料庫**，只回程序存活，避免資料庫抖動造成誤判與不必要的實例重啟；③ Cloud Run 的 startup probe 設為 `GET /health`，**新 revision 通過啟動檢查才切流量、舊 revision 保留但不接流量**（零停機輪替，且可秒級切回）；④ OPS-04 每 5 分鐘取樣，兼作保溫（降低冷啟命中率；冷啟僅 1–3 秒，**不足以造成取樣逾時**）；⑤ 部署工作流在 deploy 後輪詢 `/health` 直到 200，逾時即判失敗；⑥ **回滾為 `gcloud run services update-traffic` 切回前一 revision，秒級生效、不需重新建置**（ADR-0005） | **Gate 2 採 24 小時採樣**（Leader 對 O-009 的裁決；連續 7 天為正式環境目標，不作 Gate 2 門檻）：OPS-04 的 artifact 統計成功／總取樣次數，門檻 ≥ 99%。部署中斷時長：部署期間以每 5 秒一次輪詢記錄連續失敗時長，門檻 < 60 秒。**判讀規則**：若未達標，須先區分是應用缺陷還是平台事件，判讀依據為 **Cloud Run 內建指標**（`Request count` 依回應碼分組、`Container startup latency`、`Container instance count`）與 revision 事件紀錄，以及該時段 GitHub 排程的取樣間隔是否異常拉長（ADR-0005、`06_部署架構與CICD.md` 第 6 章） |
...
| **NFR-006** 資料持久性<br>重啟／重新部署後 100% 仍在 | **由架構保證，不依賴腳本正確性**：資料庫是外部託管的 Neon Postgres，與 Cloud Run 運算實例**完全分離**，重新部署（與 revision 回滾）只換運算容器，不觸碰資料（ADR-0002、ADR-0005）。migration 為 forward-only，不會在部署時刪表 | 建立 10 筆待辦 → 觸發一次重新部署 → 呼叫清單 API 比對 10 筆的 `id` 與 `title` 皆一致，輸出比對結果 |
...
├── infra/
│   └── cloudrun-service.yaml          # OPS-03：Cloud Run 服務宣告（IaC，只含變數名稱）
├── migrations/
...
| SD-04 | **Cloud Run 冷啟的殘留風險**：`min instances = 0` 時容器閒置後會被回收，若 OPS-04 保溫失效，下一次請求會遇到 **1–3 秒**冷啟，該次回應時間為 P95 的離群值 | **風險等級已因 T-0010 平台變更而實質下降**（原 Render Free 為 30–50 秒，見 ADR-0003；現為 1–3 秒，見 ADR-0005）。1–3 秒**不足以造成監測取樣逾時**，故不再是 NFR-003 的主要誤報來源，只影響回應時間的離群值。承擔方式不變：`min instances = 0` 是免費的必要條件，**刻意不修**（調高即視為規格變更）；NFR-001 量測規定先暖身 10 秒；`06_部署架構與CICD.md` 第 6 章仍把冷啟列為已知誤報來源，並以 Cloud Run 的 `Container startup latency` 指標佐證。**不隱藏、不假裝沒有** | dev-ops、qa-lead |
...
| 日期 | version | 任務卡 | 變更摘要 |
|---|---|---|---|
| 2026-09-19 | **0.3** | **T-0010**（規格變更請求，使用者裁決、Leader 核准） | 雲端平台由 Render 改為 **GCP Cloud Run**（＋ Artifact Registry），資料庫維持 Neon（ADR-0002 不變）。僅改四處：第 1.2 節容器圖邊界、第 2 章技術選型表的雲端列與 CI/CD 列（ADR 指向改為 ADR-0005）、第 7 章 NFR-001／002／003／006 的平台相關敘述（冷啟 30–50 秒 → 1–3 秒）、第 9 章 SD-04。**其餘章節一字未動**；第 3、5、8 章尚存的 Render 字樣屬 T-0010 授權範圍外，已於交接檔列出行號待 Leader 決定是否另開卡。決策紀錄見 `adr/ADR-0005-雲端平台-CloudRun.md` |
| 2026-09-19 | 0.3（不進版，Leader 裁決） | **T-0010 追加修正**（Leader 審核通過後授權） | 補齊上一列列為「授權範圍外」的三處：第 3.3 節 OPS-03（`infra/render.yaml` ＋ Deploy Hook → `infra/cloudrun-service.yaml` ＋ build → push Artifact Registry → `gcloud run deploy` → 輪詢 `/health`）、第 6.5 節日誌（由 Render 收集 → 由 Cloud Logging 收集）、第 8.1 節目錄結構（`render.yaml` → `cloudrun-service.yaml`）。**至此 SD 全書無 Render 專屬設計指示**，僅餘歷史對比敘述。同批次另修 `04_API規格.yaml` 的 staging `servers` 佔位符（`info.version` 0.1.1 → 0.1.2）。Leader 同時裁決：保溫維持每 5 分鐘不改；**不建立 `infra/render.yaml`** |
| 2026-09-20 | 0.3（字面同步，未變更版本號） | **T-0040 r2**（規格變更請求，Leader 2026-09-20 審核 r1 後追加 outputs） | **只動第 7 章 NFR-002① 一列**，把 HTTP→HTTPS 導向的狀態碼字面由寫死的 **301** 改為 **3xx（實測 302 Found）**：對策欄改為「由 Google 前端以 3xx 導向 HTTPS，**判準為『回 3xx 且 `Location` 為對應的 `https://` 網址』，不寫死單一狀態碼**」；驗證方式欄①由「觀察 301」改為「觀察首行為 3xx 且 `location` 標頭為對應的 `https://` 網址」，並補上可直接照抄的 `curl -sSI` 指令。依據：`24_缺陷清單.md#D-016`（qa-at r3 與 qa-lead T-0030 各自獨立實測皆為 `HTTP/1.1 302 Found`）、**Leader 2026-09-19T17:30:10 裁決①接受 302 等效**。用字與 `06_部署架構與CICD.md` §2 資源表（version 0.3）、`adr/ADR-0005-雲端平台-CloudRun.md` 決定第 7 點 T-0040 附註三方一致。**理由不寫死單一狀態碼**：導向狀態碼是 Google 前端的平台行為，團隊零程式碼可改、平台亦未承諾其不變，規格不對不受控的實作細節做承諾。**本次未新增或變更任何設計決策，故 version 維持 0.3、`status` 維持 `frozen`**；`20_測試案例.md` TC-079 的判準文字由 qa-lead 於下一輪測試計畫同步卡處理，不在本卡範圍 |
