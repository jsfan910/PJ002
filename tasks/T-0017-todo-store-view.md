...
inputs:
  - docs/specs/10_開發計畫_WBS.md#1.7（WI-07 全文：背景與限制、驗收方式草案）
...
acceptance:
  ...
  - US-001～US-008 每條 AC 在 UI 可實際操作到，交接檔逐條列「AC → 畫面／操作」
  - 錯誤狀態（4xx/5xx、網路失敗）與載入狀態有使用者可理解的顯示（AC-008-3、AC-008-4）
  - 刪除有二次確認（Q-003）；時間以本地時區 yyyy-mm-dd HH:mm 顯示（Q-004）
reviewer: dev-tl
...
實作 WBS WI-07「前端狀態與呈現（單向資料流）」。本卡的背景、限制與驗收指令以 `docs/specs/10_開發計畫_WBS.md` §1.7 為準（該節為本卡正文的一部分，必須整段讀完再開工）。
...
- 分支：以 git worktree 建立（`git worktree add "C:/Users/excal/Projects/Claude/soft-dev-team-260919-wt/{id}" -b task/{id}-{slug}`），所有操作在該 worktree 內；分支名 `task/T-0017-todo-store-view`；只在分支 commit；合併由 dev-tl 執行。禁止 add -A、--amend、reset、rebase。
- 模型：sonnet（Leader 依平衡模式指定）。
- 必須遵守：CLAUDE.md「協作協定」、角色檔 .claude/agents/dev-fe.md 的通用協定。

## 驗收方式

審核者（dev-tl）在該卡的 worktree（或自建 worktree）checkout 分支 `task/T-0017-todo-store-view`，實際重跑 WBS §1.7 的驗收指令與本卡 acceptance 逐條核對；以 `git diff main...HEAD --stat` 確認只動 outputs。
