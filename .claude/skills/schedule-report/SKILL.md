...
產出物：`docs/專案時程表_{date}.html`（白底、可深色切換、A4 橫向可列印）。內容：KPI、WBS 表（依階段分組，含派工／完成／歷時／狀態）、甘特圖（兩層：階段列／任務列 + 派工／完成／狀態欄、進度長條、審核回合斜線段、獨立里程碑列、現在時間線）。時間軸依資料自動取範圍：≤ 36 小時用小時刻度、≤ 5 天用 6 小時刻度、更長用日刻度。
