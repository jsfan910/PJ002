...
- 任務卡：`tasks/T-*.md` frontmatter（id、title、team、role、status、round、depends_on、created、updated）。
...
- 設定檔：`docs/schedule/{Epic}.json`（範例 `docs/schedule/E-001.json`）：
  ...
  - `phases`：階段順序與標籤；`phaseByTeam`：團隊預設階段；`phaseById`：個別卡的階段覆寫
...
2. **更新設定檔**：若有新的關卡／里程碑、Epic 狀態、staging 網址、新卡的階段歸屬，先改 `docs/schedule/{Epic}.json`。新卡若未在 `phaseById`，會依 `phaseByTeam` 歸到團隊預設階段。
