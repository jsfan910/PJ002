...
description: 產出可列印的專案時程表（WBS 表 + 甘特圖）HTML，資料自動取自任務卡 frontmatter 與交接檔各輪開工／完工時戳，並發布為同一個 Artifact 網址。用法：/schedule-report [E-001] [--date yyyy-mm-dd] [--no-publish]。
---

# /schedule-report — 專案時程表（WBS + 甘特圖）

產出物：`docs/專案時程表_{date}.html`（白底、可深色切換、A4 橫向可列印）。內容：KPI、WBS 表（依階段分組，含派工／完成／歷時／狀態）、甘特圖（兩層：階段列／任務列 + 派工／完成／狀態欄、進度長條、審核回合斜線段、里程碑群組每筆一列且時間軸以菱形標示、現在時間線）。時間軸依資料自動取範圍：≤ 36 小時用小時刻度、≤ 5 天用 6 小時刻度、更長用日刻度；刻度含小時時，時間軸分上下兩列（上列日期、下列時間，日界線加深）。

**「現在」線＝產表時刻**（產生器以設定檔 `tz` 換算後寫死進 HTML 的 `generatedAtLocal`），不是開啟頁面的時間；產表時刻一律納入時間軸範圍，所以「現在」線永遠可見，最後一筆完工到產表之間的空白就是「無事發生」。要更新「現在」，重新產表。
...
- 設定檔：`docs/schedule/{Epic}.json`（範例 `docs/schedule/E-001.json`）：
